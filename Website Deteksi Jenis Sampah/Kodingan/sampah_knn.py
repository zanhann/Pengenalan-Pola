# ==============================================================================
# APLIKASI KLASIFIKASI SAMPAH (KNN V3.0 - FINAL ANTI LEAKAGE)
#
# Deskripsi:
# Script ini menerapkan model KNN dengan alur pra-pemrosesan yang benar untuk
# mencegah kebocoran data, memberikan akurasi yang realistis, dan memprediksi
# jenis sampah dari gambar yang diunggah melalui antarmuka GUI.
#
# Diperlukan file:
# - sampah_features1.csv (Data asli SEBELUM normalisasi)
# ==============================================================================
...
from sklearn.metrics import accuracy_score, classification_report # Ditambahkan classification_report
...
# !!! PENTING: ATUR PATH LENGKAP DI SINI JIKA FILE TIDAK SATU FOLDER !!!
# Ganti dengan path lengkap tempat Anda menyimpan file CSV jika perlu
# Contoh: BASE_PATH = r"C:\Users\user\path\ke\folder\anda"
BASE_PATH = os.getcwd() # Menggunakan folder saat ini sebagai default
UNNORMALIZED_CSV_PATH = os.path.join(BASE_PATH, 'sampah_features1.csv')
...
# Kamus pemetaan dari jenis sampah
TRASH_TYPE_MAPPING = {
    "paper": "Kertas",
    "plastic": "Plastik",
    "metal": "Logam",
...
def train_knn_model():
    """Melatih model KNN dengan alur yang benar untuk mencegah kebocoran data."""
    global MODEL_KNN, SCALER, LABEL_ENCODER, MODEL_ACCURACY, FEATURE_COLUMNS
    
    try:
        df_full = pd.read_csv(UNNORMALIZED_CSV_PATH)
    except FileNotFoundError:
        messagebox.showerror("File Error", f"Error: File '{UNNORMALIZED_CSV_PATH}' tidak ditemukan. Pastikan path diatur dengan benar.")
        return False
        
    print("Memulai proses pelatihan model dengan metode anti-kebocoran data...")
    ...
    df_full['base_filename'] = df_full['filename'].apply(get_base_filename)
    unique_base_files = df_full['base_filename'].unique()
    base_train_files, base_test_files = train_test_split(unique_base_files, test_size=0.2, random_state=42)
    
    train_df = df_full[df_full['base_filename'].isin(base_train_files)]
    test_df_all_aug = df_full[df_full['base_filename'].isin(base_test_files)]
    test_df = test_df_all_aug[~test_df_all_aug['filename'].str.contains('_rot')]

    print(f"Pembagian data anti-kebocoran selesai:")
    print(f"Jumlah gambar asli untuk latih: {len(base_train_files)}")
    print(f"Jumlah gambar asli untuk uji: {len(base_test_files)}")
    print(f"Total baris data latih (termasuk augmentasi): {len(train_df)}")
    print(f"Total baris data uji (hanya asli): {len(test_df)}")

    FEATURE_COLUMNS = [col for col in df_full.columns if col not in ['filename', 'class', 'base_filename']]
    ...
    print("Scaler berhasil dilatih HANYA pada data latih.")
    
    MODEL_KNN = KNeighborsClassifier(n_neighbors=3, weights='distance', metric='euclidean')
    ...
    print(f"AKURASI MODEL YANG SEBENARNYA PADA TEST SET: {MODEL_ACCURACY:.2%}")
    print("="*60 + "\n")
    
    print("LAPORAN KLASIFIKASI (PERFORMA PER KELAS):\n")
...
# ==============================================================================
# BAGIAN 2: ANTARMUKA PENGGUNA (GUI) DENGAN TKINTER
# ==============================================================================

class App:
    def __init__(self, root):
        self.root = root
        self.root.title("Klasifikasi Jenis Sampah (KNN v3 - Final)")
        ...
        self.nama_daun_label = Label(details_frame, text="--", font=self.font_normal, bg=self.color_frame, fg=self.color_text, wraplength=400, justify=tk.LEFT)
        self.nama_daun_label.grid(row=0, column=1, sticky="w", pady=5)

        Label(details_frame, text="Deskripsi:", font=self.font_subtitle, bg=self.color_frame, fg=self.color_text).grid(row=1, column=0, sticky="nw", pady=5)
        self.manfaat_label = Label(details_frame, text="--", font=self.font_normal, bg=self.color_frame, fg=self.color_text, wraplength=400, justify=tk.LEFT)
        self.manfaat_label.grid(row=1, column=1, sticky="w", pady=5)

        Label(details_frame, text="Status Keyakinan:", font=self.font_subtitle, bg=self.color_frame, fg=self.color_text).grid(row=2, column=0, sticky="nw", pady=5)
        self.confidence_label = Label(details_frame, text="--", font=self.font_normal, bg=self.color_frame, fg=self.color_text)
        self.confidence_label.grid(row=2, column=1, sticky="w", pady=5)

        ttk.Separator(result_frame, orient='horizontal').pack(fill='x', padx=20, pady=10)
        self.akurasi_label = Label(result_frame, text=f"Akurasi Model (KNN): --", font=("Helvetica", 10, "italic"), bg=self.color_frame, fg="#888888")
        self.akurasi_label.pack(pady=(0, 15))
        self.root.after(200, self.update_accuracy_label)

    def update_accuracy_label(self):
        self.akurasi_label.config(text=f"Akurasi Realistis Model (KNN): {MODEL_ACCURACY:.2%}")
    ...
    def run_prediction(self, file_path):
        self.nama_daun_label.config(text="Menganalisis...")
        self.manfaat_label.config(text="Mohon tunggu...")
        self.confidence_label.config(text="")
        self.root.update_idletasks()

        new_features = extract_features_from_image(file_path)
        if new_features is None:
            self.nama_daun_label.config(text="Gagal memproses gambar.")
            self.manfaat_label.config(text="Pastikan gambar jelas & merupakan objek tunggal.")
            self.confidence_label.config(text="--")
            return

        new_features_df = pd.DataFrame([new_features])
        new_features_df = new_features_df[FEATURE_COLUMNS]
        new_features_scaled = SCALER.transform(new_features_df.values)
        
        prediction_encoded = MODEL_KNN.predict(new_features_scaled)
        prediction_proba = MODEL_KNN.predict_proba(new_features_scaled)
        
        confidence = np.max(prediction_proba) * 100
        
        # Logika ambang batas keputusan
        CONFIDENCE_THRESHOLD = 50.0  # Anda bisa menaikkan atau menurunkan nilai ini

        if confidence >= CONFIDENCE_THRESHOLD:
            predicted_trash_type = LABEL_ENCODER.inverse_transform(prediction_encoded)[0]
            predicted_description = TRASH_TYPE_MAPPING.get(predicted_trash_type, "Deskripsi tidak diketahui")
            status_keyakinan = f"{confidence:.2f}%"
            if confidence > 85: status_keyakinan += " (Sangat Yakin)"
            elif confidence > 65: status_keyakinan += " (Cukup Yakin)"
            
            self.nama_daun_label.config(text=f"{predicted_trash_type.capitalize()}")
            self.manfaat_label.config(text=f"{predicted_description}")
            self.confidence_label.config(text=status_keyakinan)
        else:
            self.nama_daun_label.config(text="Objek Tidak Dikenali")
            self.manfaat_label.config(text="Model tidak dapat mengidentifikasi gambar ini dengan keyakinan yang cukup.")
            self.confidence_label.config(text=f"{confidence:.2f}% (Keyakinan Terlalu Rendah)")
...
if __name__ == "__main__":
    if train_knn_model():
        ...
        messagebox.showerror(
            ...
            f"Pastikan file '{os.path.basename(UNNORMALIZED_CSV_PATH)}' "
            "berada di path yang benar dan bisa diakses."
        )
